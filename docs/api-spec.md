...
| 404 | 40401 | 존재하지 않는 분석 세션 **또는** 존재하지 않는 경매 ⚠️ |
...
> ⚠️ **`40401` 충돌**: `AnalysisSessionNotFoundException`과 `AuctionNotFoundException`이
> 같은 코드를 쓰고 있습니다. 프론트에서 `40401`만으로는 "분석 세션이 없다"와 "경매가 없다"를
> 구분할 수 없습니다. 어느 API를 호출했는지로 판단해야 하며, 코드 재배치가 필요합니다.
>
> `40402`는 현재 비어 있습니다.
