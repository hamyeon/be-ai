...
1순위 경로의 상태 반영은 "전체 매물 대비 비율"입니다. 시세 중앙값이 상태 불문 전체 매물에서 나온 값이라, 분모도 같은 모집단(전체 매물 계수, 실측 0.482·309건)을 씁니다. 예를 들어 `DS`(새상품)는 실측 계수 비율(0.778/0.482 ≈ 1.6배)만큼 올라가고 `B`(0.307/0.482 ≈ 0.64배)는 내려갑니다. `UNKNOWN`은 비율 1.0으로 시세 중앙값이 그대로 추천가가 됩니다.
...
| --- | --- | --- |
| `DS` | 0.778 | 실측 (당근 63건) |
| `S` | 0.70 | 기본값 (표본 12건, 부족) |
| `A` | 0.485 | 실측 (매물 설명 재분류, 107건) |
| `B` | 0.307 | 실측 (매물 설명 재분류, 57건) |
| `C` | 0.20 | 기본값 (표본 7건, 부족) |
