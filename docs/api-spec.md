...
1순위 경로의 상태 반영은 "전체 매물 대비 비율"입니다. 시세 중앙값이 상태 불문 전체 매물에서 나온 값이라, 분모도 같은 모집단(전체 매물 계수, 실측 0.444·510건)을 씁니다. 예를 들어 `DS`(새상품)는 실측 계수 비율(0.754/0.444 ≈ 1.7배)만큼 올라가고 `B`(0.333/0.444 ≈ 0.75배)는 내려갑니다. `UNKNOWN`은 비율 1.0으로 시세 중앙값이 그대로 추천가가 됩니다.

상태 등급별 계수의 출처(2026-09 실측 기준):

| 등급 | 계수 | 출처 |
| --- | --- | --- |
| `DS` | 0.754 | 실측 (당근 101건) |
| `S` | 0.70 | 기본값 (표본 25건, 부족) |
| `A` | 0.478 | 실측 (매물 설명 재분류, 166건) |
| `B` | 0.333 | 실측 (매물 설명 재분류, 103건) |
| `C` | 0.20 | 기본값 (표본 13건, 부족) |
