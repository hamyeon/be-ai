...
| 순위 | 근거 | 계산 |
| --- | --- | --- |
| 1 | **당근마켓·후르츠패밀리 중고 실거래** (모델별 10건 이상, 현재 30개 모델·1,590건) | 실거래 중앙값 × 상태비율 × 구성품 반영률. 권장 범위는 실거래 IQR(25~75% 구간) |
| 2 | KREAM/eBay 새제품 시세 (1순위 매칭이 없을 때) | (KREAM×0.7 + eBay×0.3) × 상태계수 × 구성품 반영률. 권장 범위는 ±5% |
| — | 둘 다 없으면 | `0원 + "시세 정보 없음"` |
...
1순위 경로의 상태 반영은 "일반 중고 대비 비율"입니다. 실거래 매물 대부분이 일반 중고라, 예를 들어 `DS`(새상품)는 실측 계수 비율(약 1.9배)만큼 올라가고 `C`는 내려갑니다.
...
```json
{
  ...
  "data": {
    ...
    "reason": "당근마켓·후르츠패밀리에 올라온 Dunk Low 중고 매물 116건을 근거로 계산했습니다. 실거래가 중앙값은 40,000원이고, 매물의 절반이 25,000원 ~ 81,000원 사이에 있습니다. 상품 상태 UNKNOWN(기타 상태)는 일반 중고 대비 100% 수준으로 반영했습니다. 구성품이 모두 포함되어 있어 100% 반영률을 적용했습니다. 이를 바탕으로 최종 추천가는 40,000원이며, 판매 권장 범위는 실거래 분포를 따라 25,000원 ~ 81,000원입니다.",
    "kreamMatches": [],
    "ebayMatches": []
  },
  "error": null
...
1순위 중고 시세에 매칭이 없을 때, KREAM/eBay에서 조건에 맞는 유사 거래 데이터를 조회한 뒤 각 시세와 상품 상태 및 구성품 상태를 반영하여 추천 판매가를 계산합니다.
...
```json
{
  ...
  "data": {
    ...
    "reason": "KREAM 유사 거래 1건의 평균가 114,000원과 eBay 유사 거래 50건의 평균가 137,280원을 각각 70%, 30% 비율로 반영해 기준 시세 120,984원을 계산했습니다. 상품 상태는 C(하자 있음)로 판단하여 20% 반영률을 적용했습니다. (실거래 표본이 부족해 기본값을 사용했습니다) 구성품이 모두 포함되어 있어 100% 반영률을 적용했습니다. 이를 바탕으로 최종 추천가는 24,000원으로 산정했으며, 판매 권장 범위는 23,000원 ~ 25,000원입니다. 추천가는 KREAM 평균가 대비 약 79% 낮은 수준입니다.",
    "kreamMatches": [
      {
        "source": "KREAM",
        "brand": "Nike",
