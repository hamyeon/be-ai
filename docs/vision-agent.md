...
> v1 / v2 측정 결과는 실제 키가 있는 환경에서 한 번 돌린 뒤 여기에 채운다.
...
| high가 비용만큼 값을 하는가 | 리포트의 토큰 사용량 대비 size 응답률/정확도 |

판단 기준은 정해두고 시작한다.

- `size` 응답률이 올라가면서 응답 정확도가 떨어지지 않으면 채택
- 응답률만 오르고 정확도가 떨어지면 **반려** (없는 사이즈를 지어내기 시작한 것)
- 정확도가 같은데 토큰만 늘면 반려

`THUMBNAIL_300`에서 high를 켠 결과는 특히 중요하다. 300x300 이미지에는 high로 얻을 게 없다는 게
예상인데, 그렇다면 크롤링 데이터를 쓰는 경로에서는 원본 URL을 쓰도록 백필 쪽을 손봐야 한다.
